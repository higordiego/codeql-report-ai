...
pub fn find_python_files(root_dir: &Path) -> crate::Result<Vec<std::path::PathBuf>> {
    ...
    let ignored_dirs = [
        ...
        "pydantic", "requests", "urllib3",
        "certifi", "charset_normalizer",
        "idna", "six", "packaging",
        "pyparsing", "markupsafe", "jinja2",
        "click", "itsdangerous", "werkzeug",
        "flask", "django", "fastapi",
        "uvicorn", "starlette", "pydantic",
        "typing_extensions", "annotated_types",
        "python-dateutil", "pytz", "babel",
        "markdown", "pygments", "docutils",
        "sphinx", "alabaster", "imagesize",
        "snowballstemmer", "babel", "pytz",
        "six", "docutils", "markupsafe",
        "jinja2", "pygments", "alabaster",
        ...
        .filter_map(|e| e.ok())
    {
        ...
        if contains_ignored {
            tracing::debug!("Ignorando caminho que contém diretório ignorado: {}", path.display());
            continue;
        }
    ...
    tracing::debug!("Total de arquivos Python encontrados: {}", python_files.len());
    Ok(python_files)
}
...
pub fn create_targeted_chunks(
    ...
    let mut file_results: std::collections::HashMap<String, Vec<&CodeQLResult>> = std::collections::HashMap::new();
    ...
    for file_path in files {
        ...
        tracing::debug!("Comparando arquivo encontrado: '{}' (nome: '{}')", file_str, file_name);
