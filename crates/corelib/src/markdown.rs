...
pub fn generate_report(
    ...
    // Resumo executivo
    report.push_str(&generate_executive_summary(codeql_analysis, chatgpt_analyses));
    
    // Estatísticas do CodeQL
    report.push_str(&generate_codeql_statistics(&codeql_analysis.statistics));
    
    // Achados detalhados
    report.push_str(&generate_findings_section(codeql_analysis, chatgpt_analyses));
...
fn generate_executive_summary(
    ...
    let avg_risk_score: f32 = chatgpt_analyses
        ...
        .sum::<f32>() / chatgpt_analyses.len() as f32;
...
fn generate_codeql_statistics(stats: &AnalysisStatistics) -> String {
    ...
    section.push_str(&format!("- **Total de resultados:** {}\n", stats.total_results));
    section.push_str(&format!("- **Arquivos com problemas:** {}\n", stats.files_with_issues));
    section.push_str(&format!("- **Total de arquivos com problemas:** {}\n\n", stats.files_with_issues));
    
    // Distribuição por severidade
    section.push_str("### Distribuição por Severidade\n\n");
    section.push_str(&format!("- 🔴 **Alta:** {} problemas\n", stats.high_severity_count));
    section.push_str(&format!("- 🟡 **Média:** {} problemas\n", stats.medium_severity_count));
    section.push_str(&format!("- 🟢 **Baixa:** {} problemas\n", stats.low_severity_count));
    section.push_str("\n");
...
fn generate_findings_section(
    ...
    for analysis in chatgpt_analyses {
        for finding in &analysis.findings {
            findings_by_file
                .entry(finding.file_path.clone().unwrap_or_else(|| "unknown".to_string()))
                .or_default()
                .push(finding);
        }
    }
...
fn generate_recommendations_section(chatgpt_analyses: &[ChatGPTAnalysis]) -> String {
    ...
    for (i, recommendation) in all_recommendations.iter().enumerate() {
        ...
        section.push_str(&format!("**Esforço estimado:** {}\n\n", recommendation.effort));
