...
impl Config {
    ...
    pub fn validate(&self) -> crate::Result<()> {
        if self.openai_api_key.is_empty() {
            return Err(crate::Error::Config("OPENAI_API_KEY não pode estar vazia".to_string()));
        }
        ...
        if self.temperature < 0.0 || self.temperature > 2.0 {
            return Err(crate::Error::Config("Temperatura deve estar entre 0.0 e 2.0".to_string()));
        }
        
        if self.rate_limit_rps == 0 {
            return Err(crate::Error::Config("Rate limit deve ser maior que 0".to_string()));
        }
